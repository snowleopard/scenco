...
extern "C" {
	...
	int single_literal_encoding(){
		...
		try{
			...
			safe_exit("Undefined error during single literal encoding.");
			return -1;
		}
	...
	int sequential_encoding(){
		...
		try{
			...
			safe_exit("Undefined error during sequential.");
			return -1;
		}
		return 0;
	}
	...
	int random_encoding(int num_enc){
		...
		try{
			...
			safe_exit("Undefined error during random encoding.");
			return -1;
		}
		return 0;
	}
	...
	int heuristic_encoding(int num_enc){
		...
		try{
			...
			safe_exit("Undefined error during heuristic encoding.");
			return -1;
		}
		return 0;
	}
	...
	int exhaustive_encoding(int num_enc){
		...
		try{
			...
			if (export_variables(exhaustive) != 0){
				safe_exit("Error exporting variables for external environment for exhaustive encoding.");
				return -1;
			}

		} catch(...) {
			safe_exit("Undefined error during exhaustive encoding.");
			return -1;
		}
	...
	int get_controller_formulae(char* abcPath){
		...
		try{
			if (get_formulae_nodes(abcPath) != 0){
				safe_exit("Error using ABC for getting Boolean formulae of nodes and arcs (controller synthesis).");
				return -1;
			}

			if( (err = equations_abc(cpog_count,bits)) != 0){
				safe_exit("Error merging final equations for ABC mapping (controller synthesis).");
				return -1;
			}

			freeBooleanFunctions();
		} catch(...) {
			safe_exit("Undefined error during controller generation.");
			return -1;
		}
	...
	int get_CPOG_formulae(char* abcPath){
		...
		try{
			...
			safe_exit("Undefined error during CPOG generation.");
			return -1;
		}
	...
	void push_input(char* input){
		try{
			...
			safe_exit("Undefined error on pushing input.");
			return;
		}
		return;
	}
	...
	void push_output(char* output){
		try{
			...
			safe_exit("Undefined error on pushing output.");
			return;
		}
		return;
	}
	...
	void push_equation(char* equation){
		try{
			...
			safe_exit("Undefined error on pushing equation.");
			return;
		}
		return;
	}
	...
	double map_and_get_area_circuit(char *abcPath, char *techLibrary){
		...
		try{
			...
			// run abc
			if( system(command) == -1){
				safe_exit("Error running ABC for mapping getting area of the circuit.");
				return -1;
			}

			if(parse_area_result_abc() != 0){
				safe_exit("Error parsing ABC result for getting Area of the circuit.");
				return -1;
			}

			free(command);
		} catch(...) {
			safe_exit("Undefined error during area estimation.");
			return -1;
		}
	...
	int generate_verilog(char *abcPath, char *techLibrary, char *vFile){
		...
		try{
			...
			safe_exit("Undefined error during verilog file generation.");
			return -1;
		}
		return 0;
	}
	...
	void free_formulae(){
		...
		try{
			...
			safe_exit("Undefined error while unloading formulae.");
			return;
		}
		return;
	}
	...
	int unload_graphs_codes(){
		...
		try{
			...
			safe_exit("Undefined error while unloading graphs and codes.");
			return -1;
		}
		return 0;
	}
